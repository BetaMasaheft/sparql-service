...
let $rdf := try{transform:transform($file, $local:data2rdf, ())} catch * {$err:description}
return
if($rdf[not(node())]) then (()) else 
...
let $storeRDFXML := try{xmldb:store($storecoll, $rdffilename, $rdf)} catch * {$err:description}
...
let $updateFuseki := try{updatefuseki:update($rdfxml, 'INSERT')} catch * {$err:description}
